(()=>{
 ...
 function show(session){const u=session.user;if(!u)return;
  ...
  if(nav&&!nav.querySelector('[data-shared-account]')){if(!document.querySelector('a[href="/admin/personal-gifts/"],a[href="personal-gifts/"]')&&(u.is_admin||['read','edit'].includes(u.permissions.giving))){const gifts=document.createElement('a');gifts.href='/admin/personal-gifts/';gifts.className='quiet-button';gifts.textContent='Personally received gifts';nav.append(gifts);}const a=document.createElement('a');a.dataset.sharedAccount='true';a.className='quiet-button';a.href='/admin/account/';a.textContent=u.is_admin?'My profile & users':'My profile';nav.append(a);if(u.can_switch&&u.switch_url){const a=document.createElement('a');a.className='quiet-button';a.href=u.switch_url+'?sourceOrigin='+encodeURIComponent(location.origin);a.textContent='Switch to Hope Sojourns';nav.append(a);}}
