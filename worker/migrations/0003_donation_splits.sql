...
CREATE TRIGGER donation_split_protect_parent BEFORE UPDATE ON paypal_transactions
 ...
 AND (NOT (NEW.direction='received') OR ROUND(NEW.gross*100) != (SELECT SUM(json_extract(value,'$.amountCents')) FROM donation_splits,json_each(allocations_json) WHERE entry_id=OLD.id)) BEGIN SELECT RAISE(ABORT,'Undo donor split before changing its total or type');
 END;
